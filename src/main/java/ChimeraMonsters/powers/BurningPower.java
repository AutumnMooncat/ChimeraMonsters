...
import ChimeraMonsters.actions.BurningLoseHPAction;
...
public class BurningPower extends AbstractEasyPower implements HealthBarRenderPower {
    public static final String POWER_ID = ChimeraMonstersMod.makeID(BurningPower.class.getSimpleName());
    ...
    public void onAttack(DamageInfo info, int damageAmount, AbstractCreature target) {
        if (damageAmount > 0 && target != owner && info.type == DamageInfo.DamageType.NORMAL) {
            flash();
            int amt = Math.max(1, amount / 2);
            addToTop(new ApplyPowerAction(target, this.owner, new BurningPower(target, amt), amt, true));
        }
    }
